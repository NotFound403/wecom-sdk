...
public class TmpControlDeserializer extends JsonDeserializer<TmpControl<?>> {
    ...
    public TmpControl<?> deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JacksonException {
        ...
        JsonNode config = treeNode.get("config");
        Class<?> configClazz = CONTROL_MAP.getOrDefault(control,EmptyConfig.class);
        Object configValue = ctxt.readTreeAsValue(config, configClazz);
//           ctxt.readTreeAsValue(treeNode,EmptyConfig.class)
        return new TmpControl<>(ctrlProperty, configValue);
    }
}
